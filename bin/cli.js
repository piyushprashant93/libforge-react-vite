...
var indexTs = `export { default as Button } from './Button';`;
var playgroundPackageJson = (libName) => `{
    "name": "playground",
    "private": true,
    "scripts": {
      "dev": "vite",
...
var playgroundViteConfig = `import { defineConfig } from 'vite';
  import react from '@vitejs/plugin-react';
  
  export default defineConfig({
    plugins: [react()]
  });`;
var playgroundMain = `import React from 'react';
...
var playgroundApp = (libName) => `import React from 'react';
...
var playgroundIndexHtml = `<!DOCTYPE html>
  <html lang="en">
    <head>
      <meta charset="UTF-8" />
...
var readme = `# My React Component Library
  ...
  \`\`\``;
var jsIndex = `export { default as Button } from './Button';`;
var jsMain = `import React from 'react';
import ReactDOM from 'react-dom/client';
import App from './App';
...
var jsApp = (libName) => `import React from 'react';
import { Button } from '${libName}';
...
var playgroundTsconfig = (libName) => `{
  "compilerOptions": {
    "target": "ESNext",
    "module": "ESNext",
    "moduleResolution": "Node",
...
var buttonComponent = `import React from 'react';
...
var jsButtonComponent = `import React from 'react';
...
function createTemplateFiles(targetDir, libName, useTypescript = true) {
  import_fs.default.mkdirSync(import_path.default.join(targetDir, "src"), { recursive: true });
  import_fs.default.mkdirSync(import_path.default.join(targetDir, "playground"), { recursive: true });
  const viteConfigContent = viteConfig.replace(
    "ENTRY_PLACEHOLDER",
    useTypescript ? "path.resolve(__dirname, 'src/index.ts')" : "path.resolve(__dirname, 'src/index.js')"
  );
  import_fs.default.writeFileSync(import_path.default.join(targetDir, "package.json"), packageJson.replace(/"name":\s*"my-lib"/, `"name": "${libName}"`));
  import_fs.default.writeFileSync(import_path.default.join(targetDir, "README.md"), readme);
  import_fs.default.writeFileSync(import_path.default.join(targetDir, "vite.config.ts"), viteConfigContent);
  if (useTypescript) {
    import_fs.default.writeFileSync(import_path.default.join(targetDir, "tsconfig.json"), tsconfig);
    import_fs.default.writeFileSync(import_path.default.join(targetDir, "src", "index.ts"), indexTs);
    import_fs.default.writeFileSync(import_path.default.join(targetDir, "src", "Button.tsx"), buttonComponent);
    import_fs.default.writeFileSync(import_path.default.join(targetDir, "playground", "main.tsx"), playgroundMain);
    import_fs.default.writeFileSync(import_path.default.join(targetDir, "playground", "App.tsx"), playgroundApp(libName));
    import_fs.default.writeFileSync(import_path.default.join(targetDir, "playground", "index.html"), playgroundIndexHtml);
    import_fs.default.writeFileSync(import_path.default.join(targetDir, "playground", "tsconfig.json"), playgroundTsconfig(libName));
  } else {
    import_fs.default.writeFileSync(import_path.default.join(targetDir, "src", "index.js"), jsIndex);
    import_fs.default.writeFileSync(import_path.default.join(targetDir, "src", "Button.jsx"), jsButtonComponent);
    import_fs.default.writeFileSync(import_path.default.join(targetDir, "playground", "main.jsx"), jsMain);
    import_fs.default.writeFileSync(import_path.default.join(targetDir, "playground", "App.jsx"), jsApp(libName));
    import_fs.default.writeFileSync(import_path.default.join(targetDir, "playground", "index.html"), playgroundIndexHtml.replace("/main.tsx", "/main.jsx"));
  }
  import_fs.default.writeFileSync(import_path.default.join(targetDir, "playground", "package.json"), playgroundPackageJson(libName));
  import_fs.default.writeFileSync(import_path.default.join(targetDir, "playground", "vite.config.ts"), playgroundViteConfig);
  (0, import_child_process.execSync)("npm install", { cwd: targetDir, stdio: "inherit" });
  (0, import_child_process.execSync)("npm run build", { cwd: targetDir, stdio: "inherit" });
...
async function main() {
  ...
  const targetDir = import_path2.default.join(process.cwd(), libName);
  console.log(`
\u{1F4C1} Creating project in: ${targetDir}`);
  createTemplateFiles(targetDir, libName, useTypescript);
  console.log(`
\u2705 Project created successfully!`);
  console.log(`
\u{1F449} To test your library locally:
